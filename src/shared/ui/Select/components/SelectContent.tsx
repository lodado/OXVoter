...
const SelectContent = ({
  ...
  <Portal>
    <Content
      ...
      position="popper"
      className={cn(
        `SelectContent overflow-hidden w-max z-dropdown bg-white border border-solid border-color-border-input rounded-lg shadow-dropdown mt-2`,
        contentClassName
      )}
      {...rest}
    >
      ...
      <Viewport className={cn("p-2 text-text-01", className)}>{children}</Viewport>
